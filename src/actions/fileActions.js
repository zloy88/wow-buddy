...
// Update Key/Value pair in settings object, but do not overwrite the whole object
...
export async function getWowAccountFolders(wowPath) {
  const folders = []
  const accountPath = join(wowPath, 'WTF', 'Account')

  if (existsSync(accountPath)) {
    const files = readdirSync(accountPath, { withFileTypes: true })
    files.forEach((file) => {
      if (
        file.isDirectory() &&
        file.name !== 'SavedVariables' &&
        file.name !== 'SavedVariables.bak'
      ) {
...
export async function getWowRealmFolders(wowPath, accPath) {
  const folders = []
  const fullPath = join(wowPath, 'WTF', 'Account', accPath)
...
export async function getWowCharacterFolders(wowPath, accPath, realmPath) {
  const folders = []
  const fullPath = join(wowPath, 'WTF', 'Account', accPath, realmPath)
  if (existsSync(fullPath)) {
    const files = readdirSync(fullPath, { withFileTypes: true })
    files.forEach((file) => {
      if (file.isDirectory()) {
...
export async function readREFlexData(wowPath, accPath, realmPath, charPath, region) {
  const REFlexFile = join(wowPath, 'WTF', 'Account', accPath, realmPath, charPath, 'SavedVariables', 'REFlex.lua')
...
export function getRegionFromWowPath(_app, wowPath) {
  const fullPath = join(wowPath, 'WTF', 'Config.wtf')
